...
public class CallCoordinatorTests
{
    ...
    public async Task Beschikbare_agent_wordt_gebeld_voor_wachtende_beller()
    {
        ...
        Assert.Equal(AgentStatus.Ringing, (await agents.GetAsync(T, "agent1001"))!.Status);
    }
    ...
    public async Task Pickup_van_onbekend_gesprek_faalt()
    {
        ...
        await agents.LoginAsync(T, "agent1001");
        Assert.False(await coordinator.PickupAsync(T, "agent1001", "bestaat-niet"));
    }
    ...
    public async Task Tweede_pickup_van_hetzelfde_gesprek_faalt()
    {
        ...
        Assert.True(await coordinator.PickupAsync(T, "agent1001", "caller-1"));
        Assert.False(await coordinator.PickupAsync(T, "agent1002", "caller-1")); // al uit de wacht
    }
    ...
    public async Task Opnemen_zet_beller_en_agent_in_een_mixing_brug()
    {
        ...
        Assert.Equal(AgentStatus.OnCall, (await agents.GetAsync(T, "agent1001"))!.Status);
    }
    ...
    public async Task Beller_ophangen_ruimt_gesprek_op_en_start_nawerktijd()
    {
        ...
        Assert.Equal(AgentStatus.WrapUp, (await agents.GetAsync(T, "agent1001"))!.Status);
    }
    ...
    public async Task Doorverbinden_naar_collega_belt_die_agent_en_zet_huidige_in_nawerktijd()
    {
        ...
        await agents.LoginAsync(T, "agent1002");
        await coordinator.EnqueueCallerAsync("caller-1", T, "support", "+31600000000");
        await coordinator.PickupAsync(T, "agent1001", "caller-1");
        ...
        Assert.True(await coordinator.TransferToAgentAsync(T, "agent1001", "agent1002"));
    ...
    public async Task Doorverbinden_naar_niet_ingelogde_agent_faalt()
    {
        ...
        Assert.False(await coordinator.TransferToAgentAsync(T, "agent1001", "agent1002"));
    }
    ...
    public async Task In_de_wacht_verplaatst_beller_naar_de_holding_brug()
    {
        ...
        Assert.True(await coordinator.HoldAsync(T, "agent1001"));
    ...
    {
        var (coordinator, ari, _, _) = await ActiveCallAsync();
        await coordinator.HoldAsync(T, "agent1001");
        ari.Added.Clear();

        Assert.True(await coordinator.UnholdAsync(T, "agent1001"));
    ...
    public async Task Dubbel_in_de_wacht_zetten_doet_niets()
    {
        var (coordinator, _, _, _) = await ActiveCallAsync();
        Assert.True(await coordinator.HoldAsync(T, "agent1001"));
        Assert.False(await coordinator.HoldAsync(T, "agent1001")); // al in de wacht
    }
    ...
    public async Task Hold_zonder_actief_gesprek_doet_niets()
    {
        var (coordinator, _, agents) = Build(wrapUpSeconds: 0);
        await agents.LoginAsync(T, "agent1001");
        Assert.False(await coordinator.HoldAsync(T, "agent1001"));
    }
    ...
    public async Task Koud_doorverbinden_naar_wachtrij_herrouteert_beller_en_zet_agent_in_nawerktijd()
    {
        ...
        await agents.LoginAsync(T, "agent1001"); // agent1002 niet ingelogd → beller blijft in sales wachten
        await coordinator.EnqueueCallerAsync("caller-1", T, "support", "+31600000000");
        await coordinator.TryDispatchAllAsync();
        var agentChannel = ari.Originates.Single().ChannelId;
        await coordinator.OnAgentAnsweredAsync(agentChannel);
        ...
        Assert.True(await coordinator.ColdTransferAsync(T, "agent1001", "sales"));
    ...
    public async Task Koud_doorverbinden_naar_extern_nummer_gaat_via_cc_forward()
    {
        ...
        Assert.True(await coordinator.ColdTransferAsync(T, "agent1001", "+31201234567"));
    ...
    public async Task Koud_doorverbinden_zonder_actief_gesprek_doet_niets()
    {
        var (coordinator, _, agents) = Build(wrapUpSeconds: 0);
        await agents.LoginAsync(T, "agent1001");
        Assert.False(await coordinator.ColdTransferAsync(T, "agent1001", "sales"));
    }
    ...
    public async Task Agent_die_niet_opneemt_geeft_de_beller_terug_aan_de_wacht()
    {
        ...
        Assert.Equal(AgentStatus.Available, (await agents.GetAsync(T, "agent1001"))!.Status);
    ...
    public async Task Warm_doorverbinden_belt_de_collega_en_zet_de_beller_in_de_wacht()
    {
        ...
        Assert.True(await coordinator.StartWarmTransferAsync(T, "agent1001", "agent1002"));
        ...
        Assert.Equal(AgentStatus.OnCall, (await agents.GetAsync(T, "agent1001"))!.Status);  // doorverbinder blijft in gesprek
        Assert.Equal(AgentStatus.Ringing, (await agents.GetAsync(T, "agent1002"))!.Status); // collega wordt gebeld
    }
    ...
    {
        var (coordinator, ari, agents, _) = await ActiveCallWithColleagueAsync();
        await coordinator.StartWarmTransferAsync(T, "agent1001", "agent1002");
        ...
        Assert.Equal(AgentStatus.OnCall, (await agents.GetAsync(T, "agent1002"))!.Status);
    }
    ...
    public async Task Overleg_voltooien_verbindt_de_beller_met_de_collega()
    {
        ...
        Assert.True(await coordinator.CompleteWarmTransferAsync(T, "agent1001"));
        ...
        Assert.Equal(AgentStatus.WrapUp, (await agents.GetAsync(T, "agent1002"))!.Status);
    }
    ...
    public async Task Overleg_annuleren_haalt_de_beller_terug_bij_de_agent()
    {
        ...
        Assert.True(await coordinator.CancelWarmTransferAsync(T, "agent1001"));
        ...
        Assert.Equal(AgentStatus.Available, (await agents.GetAsync(T, "agent1002"))!.Status); // collega vrij
        Assert.Equal(AgentStatus.OnCall, (await agents.GetAsync(T, "agent1001"))!.Status);
        Assert.True(await coordinator.HoldAsync(T, "agent1001")); // gesprek is weer actief en bedienbaar
    }
    ...
    public async Task Warm_doorverbinden_naar_bezette_collega_faalt_en_laat_het_gesprek_intact()
    {
        ...
        await agents.LoginAsync(T, "agent1001"); // agent1002 niet ingelogd
        await coordinator.EnqueueCallerAsync("caller-1", T, "support", "+31600000000");
        await coordinator.PickupAsync(T, "agent1001", "caller-1");
        ...
        Assert.True(await coordinator.HoldAsync(T, "agent1001")); // oorspronkelijke gesprek nog intact
    }
    ...
    public async Task Collega_neemt_overleg_niet_aan_geeft_de_beller_terug()
    {
        ...
        Assert.Contains((bridge, "caller-1"), ari.Added);                                  // beller terug bij agent1001
        Assert.Equal(AgentStatus.Available, (await agents.GetAsync(T, "agent1002"))!.Status); // collega vrij
        Assert.True(await coordinator.HoldAsync(T, "agent1001")); // gesprek weer actief
    }
    ...
    public async Task Beller_hangt_op_tijdens_overleg_ruimt_beide_legs_op()
    {
        ...
        Assert.Contains(consultChannel, ari.Hangups);  // collega eruit
        Assert.Equal(AgentStatus.WrapUp, (await agents.GetAsync(T, "agent1001"))!.Status);
        Assert.Equal(AgentStatus.Available, (await agents.GetAsync(T, "agent1002"))!.Status);
    }
    ...
    public async Task Overleg_voltooien_voordat_de_collega_opnam_faalt()
    {
        ...
        await coordinator.StartWarmTransferAsync(T, "agent1001", "agent1002");
        Assert.False(await coordinator.CompleteWarmTransferAsync(T, "agent1001")); // collega heeft nog niet opgenomen
    }
    ...
    public async Task Positie_meldingen_vertellen_elke_wachtende_zijn_plek()
    {
        ...
        await coordinator.EnqueueCallerAsync("caller-1", T, "support", "+31600000001");
        await coordinator.EnqueueCallerAsync("caller-2", T, "support", "+31600000002");
    ...
    public async Task Positie_wordt_per_wachtrij_geteld()
    {
        ...
        await coordinator.EnqueueCallerAsync("caller-1", T, "support", "+31600000001");
        await coordinator.EnqueueCallerAsync("caller-2", T, "sales", "+31600000002");
